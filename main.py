...
def main(inputs, output):
    with open("cuts/wolvescut2.txt", "r", encoding='utf-8') as f:
        txt = "".join(f.readlines())
    txt = txt.split("</s>\n")
    with open("cuts/wolveshistory.txt", "r") as f:
        cits_old = "".join(f.readlines())
    cits_old = cits_old.split("\n")
    print(txt[:5])
    ...
    def draw_text(image, txt):
        ...
        fnt = ImageFont.truetype('fonts/GOTHIC.ttf', 28)
        w, h = image.size
        if len(txt) > 20:
            st = ""
            h_offset = 0
    ...
    def save_citation(cit):
        f = open("cuts/wolveshistory.txt", "a+")
        f.write('\n'+cit)
